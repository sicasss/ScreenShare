...
Write-Host "";
Write-Host "";
Write-Host -ForegroundColor Blue "Edited by corregir, originally made by Diablos 473, all credits to them :D";
Write-Host "";
...
Write-Host -ForegroundColor Red " \     \___(  <_> )   /  / __ \|    |     \   / |    |  ";   
...
if (!(Test-Admin)) {
    Write-Warning "Brother respeta, ejecuta como admin jaja"
    Start-Sleep 10
    Exit
}
...
Catch{
    Write-Warning "Error Parseando BAM Key. Probablemente no soporta tu version de Windows jaja"
    Exit
}
